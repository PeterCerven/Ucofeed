...
export class ReviewService {
  ...
  getReviews(
    ...
    let reviews = [...this.mockReviews].filter(r => r.studyProgramId === programId);
  ...
  createReview(reviewDto: CreateReviewDto): Observable<ReviewModel> {
    ...
    // API endpoint: POST /api/public/reviews
    // Body: reviewDto

    const newReview: ReviewModel = {
      id: this.mockReviews.length + 1,
      userName: 'Current User', // Will come from auth service
      studyProgramId: reviewDto.studyProgramId,
      studyProgramName: 'Computer Science', // Will come from backend
